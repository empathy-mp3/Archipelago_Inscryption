...
namespace Archipelago_Inscryption.Components
{
    internal class SaveEntry : MonoBehaviour
    {
        ...
        internal void Init(string name, DateTime lastSaveTime, int playerCount, int locationsCount, int maxLocationsCount, int itemsCount, int maxItemsCount, int goalCount, Goal goalType, bool enableAct1, bool enableAct2, bool enableAct3, bool skipEpilogue, int version)
        {
            ...
            goalText.text = "Goal Progress: " + goalCount.ToString() + "/" + (goalType == Goal.COUNT ? "?" : ((enableAct1 ? 1 : 0) + (enableAct2 ? 1 : 0) + (enableAct3 ? 1 : 0) + (skipEpilogue ? 0 : 1)).ToString());
        }
    }
}
