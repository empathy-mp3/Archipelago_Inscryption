### 1.5.0
 - Every item now has to say how its effect is recovered if a save no longer shows it, and the build fails until it does. Recovering items was a hand-written chain of checks, so an item that wrote something to the save and was not added to that chain became quietly unrecoverable -- worth an audit to find and a player to notice, which is how the Holo Pelt, the Act 3 upgrades, Ourobot and the Skink and Ant cards were all found at once. Each item is now classified as one of: nothing to recover, counted against how many were sent, checked against the state it writes, rebuilt when a run or act starts, or deliberately never given back. Adding an item without classifying it does not compile.
 - Fixed the Skink and the Ant cards being granted once and then lost to your next Act 1 run, including an act reset. A new run rebuilds the cards you have discovered from their story events, but only the three the game knows how to rebuild -- the caged wolf, the talking wolf and the stinkbug -- and nothing at all reads the skink or ant events, so those two cards simply went. They now come back with the rest of a new run's deck. Under `randomize_deck: randomize_type` or `randomize_all` nothing changes, because there these items decide what the random pool may contain rather than what your deck contains.
 - The Archipelago data file is now written just before the game's save file rather than just after it. Both still happen in the same call, so only a crash or a failed write can separate them, but which of the two is already on disk decides what your next connect does. With the game save landing first, an item's effect could be saved while the record of receiving it was not, and the server sending it again then read as a brand new item and granted it a second time -- duplicate teeth, a duplicate pelt, a duplicate card. Writing the record first turns that around: the item's effect goes missing instead, which is exactly what the connect-time recovery pass is for.
 - Fixed the item log mode and the DeathLink override being shared by every save instead of belonging to the one you are playing. Both were stored as `static` fields that were still written to each save's file, and the save select screen reads every save on disk to list them, so whichever save happened to be listed last decided both settings for the session -- no matter which save you then loaded. Leaving an act partway through a fight also reverted them, since that reloads the file. They are now per-save, which is how they were always written; existing saves keep their stored values.
 - Fixed the three Vessel Upgrades and the Conduit Upgrade being lost for good when Act 3 was reset. They add sigils to your side deck, and resetting the act clears that list, but nothing put them back: unlike most items they leave no flag behind to check, only a longer list, so the reconnect pass had no way to notice they were gone. Missing upgrades are now counted the way Holo Pelts are, and exactly the number missing comes back. Which sigils the Vessel Upgrades give is rolled from your save, so restored ones can differ from what you had before.
 - Fixed Ourobot being lost for good when Act 3 was reset, or if its card failed to save. The reconnect pass recovers a card item by the story event recording it was found, and nothing set one for Ourobot, so there was no way to tell whether the card had ever reached your deck. It now sets the game's own `OurobotCardDiscovered` event -- which only the cabin scene ever set -- when the item is granted, and an Act 3 reset erases that event along with the rest of the act's, so the card comes back with everything else. The event is used rather than the deck itself because Act 3 lets you spend a card, and a spent Ourobot should not read as one that never arrived.
 - Fixed the Fishbot and Lonely Wizbot cards turning up in a randomized Act 3 deck before you had been sent them. Under `randomize_deck: randomize_type` or `randomize_all` the deck is rebuilt from every Tech card with a portrait, minus a list of ones that should never be dealt, and the two talking bots were not on that list -- so a holomap node could deal you a card whose item was still sitting in the multiworld, and holding the item only added a second copy of it to the pool. They are now taken out with the other cabin cards and put back only when their item has arrived, which is what the pool was always meant to mean. As in Act 1, holding the item makes the card available to the roll rather than guaranteed: it widens the deck by a slot and takes its chances. Ourobot is unaffected, being the one Act 3 card that is guaranteed, the way the caged wolf is in Act 1.
 - Fixed Act 3 handing you an extra card whenever a card item arrived before you first entered the act, or was restored by an Act 3 reset. Act 3 builds its deck once, on entering the act, and that build gives you a slot for every card item you are holding. A card granted before it ran was still in the deck when it did, so the build added its slot beside one that was already there -- an extra card each time, under both `randomize_type`/`randomize_all` and `starter_only`. Act 1 never had this because a new run rebuilds its deck from scratch and drops anything added early. A card item arriving while Act 3's build is still pending now leaves the deck to it and only records that the item was received; the build hands the card over when you enter. Unaffected: `randomize_deck: disable`, where the real card is placed and the build sees it and leaves it alone, and Ourobot, which is never substituted.
 - Fixed an Act 2 card pack being able to hand you the Salmon before its item had arrived. Act 2's packs are the base game's own, and it picks their contents from every card marked as pack filler without consulting anything Archipelago knows -- and the Salmon, alone among the four Act 2 card items, is marked that way. The Great Kraken, Bone Lord Horn and Drowned Soul are not pack cards at all, so they were never reachable there. Act 2 packs now withhold a card until its item arrives, the same as Act 1's and Act 3's. The check is scoped to packs: the Reinforcements trap picks from the same list, and the cards it puts on the board are the opponent's rather than anything you keep.
 - Fixed a card item being able to crash on arrival when there was no deck to add it to. Applying an Act 1 card item reached straight into the current run's deck without checking there was one, unlike the code that sets Act 1's starting currency, which has always guarded for it. A card arriving with no deck to receive it is now left to the act's own start, which hands over everything you are holding -- the same handling an Act 3 card gets when it arrives before that act has been entered.
 - Finding a card now gives you that card. A card item that arrived while you were playing used to be spent on a random card from the act's pool instead of the one it named, on every setting except `randomize_deck: disable`; the idea was that a randomized deck should not be handed a specific card, but in play it just reads as being told you found the Stinkbug and then not getting a Stinkbug. It now adds the card it names, the way the base game does when you first discover one. Everything around it is unchanged: what a run *opens* holding is still decided by the deck build, so under `starter_only` your starting deck is still random and each card item still only widens it by a slot, and under `randomize_type`/`randomize_all` the card is still replaced when you next arrive at a map node. One consequence is deliberate: under `starter_only` the Stinkbug and Stunted Wolf give you their card if the item arrives mid-run and nothing if it arrives between runs, since that setting replaces the starter build the base game would have restored them in. The cards remain findable in card choices and packs either way.
 - The Stinkbug, the Stunted Wolf, the Fishbot, the Lonely Wizbot and Ourobot can now be found as well as given. None of the five is a card the base game ever offers at a choice node, so Archipelago's copy could only ever be handed to you -- and if a randomized deck rolled it away, that was the end of it for the run. They are now choosable, which puts them in every base game pool at once: card choices after a battle, holomap card choices, and card packs. The Caged Wolf is left as it was, since it is placed in your deck outright. All five are also now one per deck, so no route can give you a second copy, and none of them can turn up at all until its item has arrived -- a card choice will not hand you what a check is still paying for.
 - Card packs can now contain the cards Archipelago hands out, instead of only what the base game offers at a choice node. The Stinkbug and Stunted Wolf in Act 1, and the Fishbot and Lonely Wizbot in Act 3, carry no choice-node category, so no pack could ever hold one however many you opened; once its item arrives each now takes a roll per card in the pack, at the odds it would have had as an ordinary member of that act's pool. So one the starting deck's roll passed over is no longer gone for the run -- a later pack can still turn it up. These cards are unique, so one already in your deck is never offered again. The caged wolf and Ourobot are unaffected, being the cards their acts guarantee. Packs also now apply the same withholding the deck randomizer does, so a card whose item has not arrived cannot reach one by any route.
 - Fixed the bee figurine's story event having no reconnect-time recovery when it came from a second Progressive Squirrel, unlike the Bee Figurine item it replaces.
 - Reapplying items after leaving an act mid-fight no longer stops at the first failure. That pass runs while the scene is being torn down, and applying an item can touch things that are already going away, so one failure abandoned every item after it and skipped the save that would have kept the ones before it. Each item is now applied on its own and a failure is logged and stepped over.
 - Fixed a bug where connecting to a slot could hand you several duplicate Holo Pelts. Pelts get spent -- the Act 3 trader takes one and gives a tarot back -- so unlike most items there is no flag to look at on reconnect, and the recovery pass instead treats the pelts you are holding plus the tarots you have collected as the pelts your save still accounts for, comparing that against how many the server has sent. That comparison was made once per received pelt, while nothing it queued was applied until the whole pass had finished, so every pelt saw the same shortfall and all of them were granted again: a single missing pelt handed back five. The shortfall is now closed by exactly as many pelts as are missing, so a save that has lost one gets one back, while a freshly reset Act 3 -- which legitimately accounts for none of them -- still gets the full set. The same pass runs when leaving an act partway through a fight, so it could duplicate them there too, silently. A pelt can go missing in the first place by leaving the trade after the trader has taken it but before its check card is claimed, which the next connect now recovers from rather than compounding.
 - Fixed a regression from 1.4.5 where collecting a check during an Act 1 fight and then leaving the act marked that fight's map node as already beaten, so it was skipped when you came back. Any save taken mid-fight does this, because the game commits `currentNodeId` when a battle *starts* rather than when it ends and records nothing about whether the node was resolved -- which is why 1.4.4 already made the post-check save wait for the battle to finish. 1.4.5 then fixed a crash in that wait by also accepting a destroyed TurnManager as the battle being over, but the TurnManager is destroyed both when a battle ends and when the scene unloads with the fight still unresolved, so the deferred save could land from the menu while still holding mid-fight state. Leaving an act partway through a fight now does exactly what closing the game mid-fight has always done: the queued save is abandoned, and returning to the start screen reverts both the game save and the Archipelago data to what is on disk, so the fight is discarded and its node is waiting for you when you come back. Only *entering* an act reloaded the save previously, which is why nothing caught this on the way out. Reverting alone would cost you anything Archipelago had sent during that fight, so every item the server has sent is then replayed against the reverted data: items it already accounts for are left alone, and ones the revert dropped are granted again, silently and without the usual per-item delay. What you did during the abandoned fight is therefore discarded -- the node, spent consumables, rigged draws, teeth from overkill -- while everything Archipelago gave you survives, currency included. Offline there is nothing to replay from, so those items come back on your next connect instead. Acts 2 and 3 needed no separate handling, as this reverts whatever the act had left unsaved regardless of which act it was.
 - Act 3 card packs now work like Act 1's: the pile sits beside the rulebook on the factory wall, and opening one lays its three cards out in front of you to pick from where you stand, rather than only appearing while you look at your deck at the table. Act 3 has no card pack art of its own, so its packs are stacks of disk cards.
 - You can now leave the Act 3 table straight away, instead of being handcuffed to it until the holomap loses power partway through the intro. This needs `act3_overhaul`, and it means the holomap power-out cutscene no longer plays.
 - Act 1 card packs are now opened from the cabin rather than the card table. The pile sits beside the rulebook on the shelf, and opening one lays its three cards out in front of you to pick from where you stand, instead of sending you to the table to make the choice. Previously the pile only appeared while you were looking at your active deck at the table. Packs offer the same cards a card choice node would, and which cards a pack holds is fixed by your save, so reloading will not reroll it -- though starting a new run, including resetting the act, deals fresh packs just as it deals a fresh starting deck. Act 3 packs are unchanged for now.
 - Currency and card packs now belong to a specific act. Previously one Currency item paid all three acts at once, and card packs came from a single pool that any act could spend from, so resetting an act could neither give back what that act had spent nor avoid handing back what the other two had. Each act now has its own currency and packs, and each currency item is worth three, so the amount you receive per act is unchanged. Act 1 currency also survives death now: teeth live on the current run, so a new run starts with the full amount Archipelago has sent you instead of losing whatever was granted during the previous one. This needs a matching apworld -- seeds generated with an older one will not work.
 - Added a way to reset a single act from the Archipelago options menu, leaving the other two and all of your checks alone. Pick an act and confirm three times, and its save data is wiped and rebuilt to the state a brand new save would reach once its items arrive. Act 1's menu cards changed to suit: the separate "New Act 1 Run" card is gone, and Act 1 now reads "Start Act 1" or "Continue Act 1" like the other two acts do.
 - The connect screen now remembers the host name, port, slot name and password from your last successful connection and prefills them when you create a new save, instead of falling back to the defaults every time. Existing saves still prefill from their own last-used details, since those are the better guess. The remembered values live in `Archipelago_Inscryption.cfg` under `[Connection]`, so they survive deleting or resetting saves -- note the password is stored there as plain text.
 - Fixed a crash while building a new save file, caused by the mod setting up Kaycee's Mod challenges before the save's ascension data existed. It was harmless at startup but broke anything relying on save creation finishing -- most visibly "Reset Save Data", which reset the save but then left you sitting in the options menu instead of returning to the start screen.
 - "Reset Save Data" now also clears the Archipelago data for the save you're playing, and drops you back at the Archipelago menu ready to connect, rather than the vanilla start screen. Previously it deleted only the game's save file, leaving that slot's checks, received items and connection details behind, and left you connected to a multiworld with no matching save. Only the save you're currently playing is affected; other saves are untouched.
 - Fixed a bug where finishing Act 2 sent you into Act 3 even when Act 3 wasn't available to you. The end of Act 2 loads Act 3 unconditionally, without consulting your options, so it happened both when Act 3 was disabled entirely and when it was still locked behind its item under `act_unlocks: items`. It now returns you to the act select menu instead. Sequential act unlocks are unaffected, since finishing Act 2 is exactly what unlocks Act 3 there. Under `act_unlocks: open`, finishing Act 2 now always returns you to the act select menu as well, even if Act 3 is already available -- which act you play next isn't something finishing Act 2 should decide there. Only the transition at the end of Act 2 is redirected: choosing Act 3 from the act select menu, or a deathlink reloading it, still enters Act 3 as normal.
### 1.4.5
