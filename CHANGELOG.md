...
- Change: A number of logic settings have been changed for consistency and fairness
    ...
    - Act 1 battles are gated on a difficulty check -- roughly, how strong your deck is expected to be before logic asks you to fight. All Totem Battles only turns regular map battles into totem battles -- Boss Totems is the separate challenge that puts a totem on a boss -- so it is not supposed to lower that bar for a boss fight, but on the Prospector and the Angler it was doing so whenever Progressive Grizzlies were also missing. Logic now expects a stronger deck for those two fights in that case
