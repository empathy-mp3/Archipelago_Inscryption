### 1.5.0
 ...
 - Fixed Ourobot being lost for good when Act 3 was reset, or if its card failed to save. Every other card item has a story event recording it was found, which is what the reconnect pass looks at; Ourobot has none, so nothing could tell whether the card had ever reached your deck. All three Act 3 card items are now checked against the deck itself, which is safe because nothing else takes cards back out of an Act 3 deck. This only applies when the Act 3 deck is not being randomized: under `randomize_deck: randomize_type` or `randomize_all` the deck is rebuilt from a random pool whenever you pick a holomap node, and there these items decide what that pool may contain rather than what your deck contains, so what is in the deck says nothing about them.
